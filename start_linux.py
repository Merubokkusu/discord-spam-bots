...
# @last-modified Tue Mar 05 2019 02:22:21 GMT-0500 (Eastern Standard Time)
#
...
if in_pick == 1:
    if os.path.exists('text.txt'):
        for token in userToken:
            p = subprocess.Popen([pythonCommand,'bots/server/discord_text_spam.py',token,'null',proxy_list[proxy_number]],shell=False)
            proxy_number += 1
            sleep(1)
    else:
        if spam_text == None:
        ...
        for token in userToken:
            p = subprocess.Popen([pythonCommand,'bots/server/discord_text_spam.py',token,spam_text,proxy_list[proxy_number]],shell=False)
            proxy_number += 1            
            sleep(1)

if in_pick == 2:
    for token in userToken:
        p = subprocess.Popen([pythonCommand, 'bots/server/discord_image_spam.py', token,proxy_list[proxy_number]],shell=False)
        proxy_number += 1
            
if in_pick == 3:
    for token in userToken:
        p = subprocess.Popen([pythonCommand,'bots/server/discord_insult_spam.py', token,proxy_list[proxy_number]],shell=False)
        proxy_number += 1
...
if in_pick == 4:
    if os.path.exists('text.txt'):
        if not os.path.exists('dm_spam_text.txt'):
            ...
            file.close()
        for token in userToken:
            p = subprocess.Popen([pythonCommand,'bots/DM/discord_text_spam_dm.py',token,'null',proxy_list[proxy_number]],shell=False)
            proxy_number += 1
            sleep(2.5)
    else:
        if not os.path.exists('dm_spam_text.txt'):
        ...
        if spam_text == None:
            spam_text = input("Write spam text : ")
        for token in userToken:
            p = subprocess.Popen([pythonCommand,'bots/DM/discord_text_spam_dm.py',token,spam_text,proxy_list[proxy_number]],shell=False)
            proxy_number += 1
            sleep(2.5)
...
if in_pick == 5:
    if not os.path.exists('dm_spam_image.txt'):
        ...
        file.close()
    for token in userToken:
        p = subprocess.Popen([pythonCommand, 'bots/DM/discord_image_spam_dm.py', token,proxy_list[proxy_number]],shell=False)
        proxy_number += 1
...
if in_pick == 6:
    if not os.path.exists('dm_spam_insult.txt'):
        ...
        file.close()
    for token in userToken:
        p = subprocess.Popen([pythonCommand,'bots/DM/discord_insult_spam_dm.py', token,proxy_list[proxy_number]],shell=False)
        proxy_number += 1
