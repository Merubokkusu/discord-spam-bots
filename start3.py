...
if in_pick == 1:
    if os.path.exists('text.txt'):
        for token in userToken:
            p = subprocess.Popen(['python3','bots\server\discord_text_spam.py',token,'null',proxy_list[proxy_number]],shell=True)
            proxy_number += 1
            sleep(1)
    else:
        spam_text = input("Write spam text : ")
        for token in userToken:
            p = subprocess.Popen(['python3','bots\server\discord_text_spam.py',token,spam_text,proxy_list[proxy_number]],shell=True)
            proxy_number += 1            
            sleep(1)

if in_pick == 2:
    for token in userToken:
        p = subprocess.Popen(['python3', 'bots\server\discord_image_spam.py', token,proxy_list[proxy_number]],shell=True)
        proxy_number += 1
            
if in_pick == 3:
    for token in userToken:
        p = subprocess.Popen(['python3','bots\server\discord_insult_spam.py', token,proxy_list[proxy_number]],shell=True)
        proxy_number += 1
...
if in_pick == 4:
    if os.path.exists('text.txt'):
        if not os.path.exists('dm_spam_text.txt'):
            ...
            file.close()
        for token in userToken:
            p = subprocess.Popen(['python3','bots\DM\discord_text_spam_dm.py',token,'null',proxy_list[proxy_number]],shell=True)
            proxy_number += 1
            sleep(2.5)
    else:
        if not os.path.exists('dm_spam_text.txt'):
        ...
        spam_text = input("Write spam text : ")
        for token in userToken:
            p = subprocess.Popen(['python3','bots\DM\discord_text_spam_dm.py',token,spam_text,proxy_list[proxy_number]],shell=True)
            proxy_number += 1
            sleep(2.5)
...
if in_pick == 5:
    if not os.path.exists('dm_spam_image.txt'):
        ...
        file.close()
    for token in userToken:
        p = subprocess.Popen(['python3', 'bots\DM\discord_image_spam_dm.py', token,proxy_list[proxy_number]],shell=True)
        proxy_number += 1
...
if in_pick == 6:
    if not os.path.exists('dm_spam_insult.txt'):
        ...
        file.close()
    for token in userToken:
        p = subprocess.Popen(['python3','bots\DM\discord_insult_spam_dm.py', token,proxy_list[proxy_number]],shell=True)
        proxy_number += 1
...
if in_pick == 0:
    for token in userToken:
        if ':' in token:
            enp = token.split(':')
            p = subprocess.Popen(['python3','bots/misc/joinServer.py',enp[0],enp[1],inviteLink,useBrowser,proxy_list[proxy_number]],shell=True)
            proxy_number += 1        
            sleep(joinSpeed)
        else:
